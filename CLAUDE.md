...
> **현재 상태: Ubuntu 26.04 에서 동작 · deb 배포 중.** 22.04·24.04 는 미검증(연기).
> 제품 정의는 v0.3 에서 "보면서 배우는 도구"로 넓어졌다 — 재생·자막에 더해 메모·핀·구간
> 반복·클립 내보내기까지가 범위다. 기획서는 [`docs/spec/`](docs/spec/) 에 버전별로 쌓인다.

## 구성 지도

| 구성요소 | 위치 | 역할 |
|---|---|---|
| UI | `src/bora/window.py` | GTK4 + libadwaita. 위젯은 **libadwaita 1.1 범위**를 지킨다 |
| 재생 엔진 | **외부 libmpv** | 디코딩·하드웨어 가속·렌더·싱크. **직접 구현하지 않는다** |
| 엔진 바인딩 | `src/bora/player.py` · `glarea.py` | python-mpv(ctypes) ↔ GtkGLArea 렌더 컨텍스트 |
...
- **무거운 선택 기능을 본체에 끌어들이지 않는다** — 별도 venv·별도 프로세스로 뗀다.
  (why: STT 모델만 436 MB 다. 재생만 하려는 사람에게 그 비용을 물리지 않는다.)
- **조용한 실패를 만들지 않는다.** 외부 도구가 오류 없이 잘못된 결과를 내는 자리를 찾아
  우리가 먼저 판정한다 (자막 인코딩, 클립 이어붙이기의 코덱 대조).
  (why: 두 번 다 실측에서 ffmpeg 이 '성공'하며 깨진 결과를 냈다. 사용자는 한참 뒤에 발견한다.)
